...
// ---------- OWNER NUMBER (add to .env) ----------
const OWNER_NUMBER = process.env.OWNER_NUMBER; // e.g., '15551234567'
...
client.on('ready', async () => {
  ...
  // Send welcome message to owner (if set)
  if (OWNER_NUMBER) {
    const helpText = `🤖 *Your WhatsApp Bot is online!*\n\n` +
      ...
      `\nAuto‑reply is ON: anyone who messages the bot will get an AI response, and you'll receive a forward.`;
...
async function generateAIReply(chatId, userText) {
  ...
  try {
    ...
    return '😅 Sorry, I had a small glitch. Please try again later.';
  }
}

// ---------- MAIN MESSAGE HANDLER (Auto‑Reply + Owner Control) ----------
client.on('message', async (message) => {
  ...
  if (isOwner) {
    ...
    // If owner sends a plain message, treat as AI chat with the bot
    // (or you can make it reply to last forwarded chat – but here we just chat with AI)
    const reply = await generateAIReply(chatId, userText);
    await message.reply(reply);
    return;
  }

  // ---------- NON-OWNER MESSAGES (Auto‑Reply + Forward to Owner) ----------
  // Forward to owner (if set)
  if (OWNER_NUMBER) {
    try {
